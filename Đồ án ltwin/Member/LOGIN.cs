...
namespace Đồ_án_ltwin
{
    class LOGIN
    {
        ...
        public string totalMaleMember()
        {
            return execCount("SELECT COUNT(*) FROM Login WHERE gender='Male'");
        }
        public string totalFemaleMember()
        {
            return execCount("SELECT COUNT(*) FROM Login WHERE gender='Female'");
        }
        ...
        public bool insertAd(int id, string user, string pass, string fname, string lname, DateTime bdate, MemoryStream picture, string gender, string phone)
        {
            SqlCommand command = new SqlCommand("INSERT INTO QL(id,username,password,fname,lname,bdate,picture,gender,phone)" +
                "VALUES (@id,@us,@ps,@fn,@ln,@bdt,@pic,@gdr,@phn)", mydb.getConnection);
            command.Parameters.Add("@id", SqlDbType.Int).Value = id;
            command.Parameters.Add("@us", SqlDbType.VarChar).Value = user;
            command.Parameters.Add("@ps", SqlDbType.VarChar).Value = pass;
